 ...
 *
 * Filtered view of HocVien where khoaQuanLy IS NOT NULL (có cơ cấu đơn vị quân sự).
 * Reuses /api/education/students?studentType=military
 */
...
import { Shield, Search, ChevronLeft, ChevronRight, ExternalLink } from 'lucide-react';

// ─── Types ────────────────────────────────────────────────────────────────────

interface MilitaryStudent {
  id: string;
  maHocVien: string;
  hoTen: string;
  lop: string | null;
...
export default function MilitaryStudentsPage() {
  ...
  const [students, setStudents]   = useState<MilitaryStudent[]>([]);
  ...
  }, [page, search, filterStatus, filterKhoaHoc]);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-green-700 via-emerald-700 to-teal-700 text-white p-6 rounded-xl">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <Shield className="h-8 w-8" />
            <div>
              <h1 className="text-2xl font-bold">Học viên Quân sự</h1>
              <p className="text-sm text-white/80">
                Quản lý học viên có cơ cấu đơn vị quân sự (Đại đội / Trung đội)
              </p>
            </div>
          </div>
      ...
      <Card>
        <CardContent className="p-4">
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
            ...
            <Select value={filterKhoaHoc || 'ALL'} onValueChange={v => { setFilterKhoaHoc(v === 'ALL' ? '' : v); setPage(1); }}>
              <SelectTrigger className="w-[150px]"><SelectValue placeholder="Khóa" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="ALL">Tất cả khóa</SelectItem>
                {['K47', 'K48', 'K49', 'K50', 'K51'].map(k => (
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="h-40 flex flex-col items-center justify-center gap-2 text-muted-foreground">
              ...
              <span>Không có học viên quân sự nào phù hợp bộ lọc</span>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Đại đội / Trung đội</TableHead>
              ...
              <TableBody>
                {students.map(s => (
                  <TableRow key={s.id}>
                    ...
                    <TableCell className="text-sm">{s.khoaQuanLy ?? '—'}</TableCell>
                    <TableCell className="text-sm">
                      <div>{s.daiDoi ?? '—'}</div>
                      <div className="text-xs text-muted-foreground">{s.trungDoi ?? ''}</div>
                    </TableCell>
                    <TableCell>
      ...
      {meta.totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <span>Trang {page}/{meta.totalPages} · Tổng {meta.total} học viên</span>
