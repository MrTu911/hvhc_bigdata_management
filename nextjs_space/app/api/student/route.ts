...
export async function GET(request: NextRequest) {
  try {
    ...
    const { user, scopedOptions, response } = await requireScopedFunction(request, STUDENT.VIEW);
    if (!user) {
      return response!;
    }
    ...
    // Sử dụng StudentService với scope filtering
    ...
    // Scope options - default to ACADEMY scope if not provided
