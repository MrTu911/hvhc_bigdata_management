 ...
 * 2. Mỗi menu item (lá) phải có `functions` array
 * 3. Sidebar dùng filterMenu để lọc theo permissions
 * 4. Không dùng string literal cho function codes
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * CẤU TRÚC IA v9 — 14 nhóm cấp 1, hỗ trợ menu con 3 cấp (group → sub-section → item)
 * Một "sub-section" là MenuItem KHÔNG có `href`, có `children` và `functions: []`.
 *
 * --- LỚP 1: TỔNG QUAN ---
 ...
 *  6. CSDL Thi đua — Khen thưởng        nav.awardsDatabase
 *  7. CSDL Bảo hiểm Xã hội              nav.insuranceDatabase
 ...
 * 10. Quy trình & Văn bản số            nav.workflowDocuments
 * 11. Mẫu biểu & Xuất dữ liệu           nav.templateExport
 * 12. Phân tích, Báo cáo & AI           nav.analyticsAI
 * 13. Khai thác & Hạ tầng Dữ liệu       nav.bigdataInfra  (gộp BigData + Hạ tầng/Governance)
 * --- LỚP 5: QUẢN TRỊ ---
 * 14. Quản trị Hệ thống                 nav.systemAdmin   (Cài đặt tài khoản gom vào Cá nhân)
 *
...
export interface MenuItem {
  ...
  href?: string;          // bỏ trống nếu là sub-section (nhóm con chứa children)
  icon: LucideIcon;
  gradient?: string;
  badge?: string;
  functions: string[];    // Required function codes (ANY match = show). Sub-section dùng [].
  children?: MenuItem[];  // Có children => là sub-section (cấp 2). Children là item lá.
}
...
// Sub-section helper — giảm lặp khi khai báo nhóm con (cấp 2).
function subSection(name: string, icon: LucideIcon, children: MenuItem[]): MenuItem {
  return { name, icon, functions: [], children };
}

// ============================================================================
// MENU CONFIGURATION - Single Source of Truth
// IA v9 — 14 nhóm cấp 1 / 5 lớp nghiệp vụ, có menu con 3 cấp
// ============================================================================
export const MENU_CONFIG: MenuGroup[] = [
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // LỚP 2: CÁ NHÂN — gom TẤT CẢ chức năng cá nhân hóa vào 1 nhóm duy nhất
  // ─────────────────────────────────────────────────────────────────────────
  ...
  // ========== 2. KHÔNG GIAN CÁ NHÂN ==========
  {
    ...
    items: [
      ...
      subSection('nav.myResearchSpace', FlaskConical, [
        {
          name: 'nav.myResearch',
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // LỚP 3: CÁC CSDL NGHIỆP VỤ
  // ─────────────────────────────────────────────────────────────────────────

  // ========== 3. CSDL CÁN BỘ, QUÂN NHÂN ==========
  {
    title: 'nav.personnelDatabase',
    items: [
      subSection('nav.personnelOverviewGroup', BarChart3, [
        {
      ...
      subSection('nav.personnelStaffGroup', Users, [
        {
      ...
      subSection('nav.personnelPlanningGroup', Target, [
        {
      ...
      subSection('nav.personnelInputGroup', FileUp, [
        {
  ...
  // ========== 4. CSDL CHÍNH TRỊ — ĐẢNG VIÊN ==========
  {
    ...
    items: [
      subSection('nav.partyOverviewGroup', Network, [
        {
      ...
      subSection('nav.partyDevelopmentGroup', Workflow, [
        {
      ...
      subSection('nav.partyActivityGroup', CalendarDays, [
        {
      ...
      subSection('nav.partyEvalGroup', ClipboardCheck, [
        {
      ...
      subSection('nav.partyTransferGroup', GitMerge, [
        {
      ...
      subSection('nav.partyMemberGroup', Users, [
        {
  ...
  // ========== 8. CSDL GIÁO DỤC — ĐÀO TẠO ==========
  {
    ...
    items: [
      subSection('nav.educationOverviewGroup', BookMarked, [
        {
      ...
      subSection('nav.facultyManagementGroup', Users, [
        {
      ...
      subSection('nav.studentManagementGroup', GraduationCap, [
        {
      ...
      subSection('nav.educationProgramGroup', LibraryBig, [
        {
      ...
      subSection('nav.educationExamGroup', ClipboardList, [
        {
  ...
  // ========== 9. CSDL KHOA HỌC & CÔNG NGHỆ (gộp M09 Nghiên cứu + M20–M26 Quản lý KH) ==========
  {
    title: 'nav.scienceTech',
    items: [
      subSection('nav.researchSubGroup', FlaskConical, [
        {
      ...
      subSection('nav.scienceActivitiesGroup', FolderKanban, [
        {
      ...
      subSection('nav.scienceResourceGroup', Award, [
        {
      ...
      subSection('nav.scienceFinanceGroup', Wallet, [
        {
      ...
      subSection('nav.scienceDataGroup', Database, [
        {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // LỚP 4: NỀN TẢNG & VẬN HÀNH (dùng chung — không thuộc một CSDL nghiệp vụ)
  // ─────────────────────────────────────────────────────────────────────────

  // ========== 10. QUY TRÌNH & VĂN BẢN SỐ ==========
  {
  ...
  // ========== 12. PHÂN TÍCH, BÁO CÁO & AI ==========
  {
    title: 'nav.analyticsAI',
    items: [
      subSection('nav.analyticsReports', PieChart, [
        {
          name: 'nav.analytics',
      ...
      subSection('nav.aiAssistantGroup', Brain, [
        {
          name: 'nav.aiAdvisor',
  ...
  // ========== 13. KHAI THÁC & HẠ TẦNG DỮ LIỆU (gộp BigData + Hạ tầng/Governance) ==========
  {
    title: 'nav.bigdataInfra',
    items: [
      subSection('nav.bigdataPlatform', LayoutDashboard, [
        {
      ...
      subSection('nav.dataManagement', Database, [
        {
      ...
      subSection('nav.securityGovernanceGroup', Shield, [
        {
  ...
  // ========== 14. QUẢN TRỊ HỆ THỐNG ==========
  {
    ...
    items: [
      subSection('nav.adminAccountGroup', UserCog, [
        {
      ...
      subSection('nav.adminOrgGroup', Building2, [
        {
      ...
      subSection('nav.adminInfraGroup', HardDrive, [
        {
          name: 'nav.apiGateway',
      ...
      subSection('nav.adminMonitorGroup', Activity, [
        {
...
// ============================================================================
// HELPER FUNCTIONS — children-aware (hỗ trợ menu con 3 cấp)
// ============================================================================

/**
 * Trả về tất cả item LÁ (có href) trong một cây items, đệ quy qua children.
 * Dùng cho: đếm số mục, thu thập function codes, preview phẳng.
 */
export function flattenMenuItems(items: MenuItem[]): MenuItem[] {
  const leaves: MenuItem[] = [];
  for (const item of items) {
    if (item.children && item.children.length > 0) {
      leaves.push(...flattenMenuItems(item.children));
    } else {
      leaves.push(item);
    }
  }
...
function filterMenuItem(item: MenuItem, functionSet: Set<string>, seenHrefs: Set<string>): MenuItem | null {
  // Sub-section (có children): filter children, bỏ nếu rỗng sau filter
  if (item.children && item.children.length > 0) {
  ...
  // Item lá — khử trùng theo href (lần đầu tiên thắng)
...
/**
 * Lấy tất cả function codes cần thiết để hiện một menu item (theo href)
 */
export function getRequiredFunctions(href: string): string[] {
  for (const group of MENU_CONFIG) {
    const leaf = flattenMenuItems(group.items).find(i => i.href === href);
    if (leaf) {
...
/**
 * Lấy danh sách tất cả function codes được dùng trong menu (đệ quy children)
 */
export function getAllMenuFunctions(): string[] {
  const functions: Set<string> = new Set();
  for (const group of MENU_CONFIG) {
    for (const leaf of flattenMenuItems(group.items)) {
      leaf.functions.forEach(f => functions.add(f));
    }
  }
...
/**
 * Map từ function code → menu items (đệ quy children)
 */
export function getFunctionMenuMap(): Map<string, { group: string; item: string; href: string }[]> {
  ...
  for (const group of MENU_CONFIG) {
    for (const leaf of flattenMenuItems(group.items)) {
      for (const fn of leaf.functions) {
        if (!map.has(fn)) {
          map.set(fn, []);
        }
        map.get(fn)!.push({
